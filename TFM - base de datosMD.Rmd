...
knitr::opts_chunk$set(echo = TRUE , comment = NULL, warning = FALSE)
```
...
escalaZ <- function (x){
  media <- mean(x)
  desvst <- sd(x)
  valor_esc <- (x-media)/desvst
  return(valor_esc)
}

#Base de datos Min-Max
BD.MM <- data.frame(lapply(BD.0[,-c(2,11)], min_max_norm))
...
boxplot(BD.z[,-c(10,11)], las=2, col="lightsalmon2", main="Variables numéricas puntuación Z")
```
...
# SVM - ENN + Submuestreo y Preprocesado range
set.seed(1234567)
SVM_D <- train(Condicion ~ ., data = BD_Ptrain3, method = "svmLinear",
              trControl = ctrl,
              #preProcess = c("center","scale"),
              preProcess = "range",
...
set.seed(1234567)
RF_D <- train(Condicion ~., data=BD_Ptrain3, method='rf', 
             trControl = ctrl,
             preProcess = c("center","scale"),
             #preProcess = "range",
             metric='Accuracy',
...
set.seed(1234567)
ANN_D <- train(Condicion ~ ., data = BD_Ptrain2, method = "nnet",
              trControl = ctrl,
              preProcess = c("center","scale"),
              #preProcess = "range",
...
```{r}
SVM_PRED_D <- predict(SVM_D, newdata = BD_Ptest3 )
CM_SVMDEF <- confusionMatrix(SVM_PRED_D, BD_Ptest3$Condicion)

RF_PRED_D <- predict(RF_D, newdata = BD_Ptest3 )
CM_RFDEF <- confusionMatrix(RF_PRED_D, BD_Ptest3$Condicion)

ANN_PRED_D <- predict(ANN_D, newdata = BD_Ptest2 )
CM_ANNDEF <- confusionMatrix(ANN_PRED_D, BD_Ptest2$Condicion)
```
...
BD_Ptrain3red <- BD_Ptrain3[,c(1,3,5,6,7,11)]
BD_Ptest3red <- BD_Ptest3[,c(1,3,5,6,7,11)]


BD_Ptrain2red <- BD_Ptrain2[,c(1,3,5,6,7,11)]
BD_Ptest2red <- BD_Ptest2[,c(1,3,5,6,7,11)]


# SVM - ENN + Submuestreo y Preprocesado range

set.seed(1234567)
SVM_red <- train(Condicion ~ ., data = BD_Ptrain3red, method = "svmLinear",
              trControl = ctrl,
              #preProcess = c("center","scale"),
              preProcess = "range",
              tuneGrid = expand.grid(C= 4)
...
# ANN - ENN y center + scale
set.seed(1234567)
ANN_red <- train(Condicion ~ ., data = BD_Ptrain2red, method = "nnet",
              trControl = ctrl,
              preProcess = c("center","scale"),
              #preProcess = "range",
              tuneGrid = expand.grid(size= 9, decay = 0.2)
...
```{r}
SVM_PREDred <- predict(SVM_red, newdata = BD_Ptest3red )
CM_SVMred <- confusionMatrix(SVM_PREDred, BD_Ptest3red$Condicion)

ANN_PREDred <- predict(ANN_red, newdata = BD_Ptest2red )
CM_ANNred <- confusionMatrix(ANN_PREDred, BD_Ptest2red$Condicion)
```
...
|---	|---	|---	|---|
|Precisión|`r round(CM_SVMDEF$overall["Accuracy"],3)`|`r round(CM_RFDEF$overall["Accuracy"],3)`|`r round(CM_ANNDEF$overall["Accuracy"],3)`|
|Sensibilidad|`r round(CM_SVMDEF$byClass["Sensitivity"],3)`|`r round(CM_RFDEF$byClass["Sensitivity"],3)`|`r round(CM_ANNDEF$byClass["Sensitivity"],3)`|
|Especificidad|`r round(CM_SVMDEF$byClass["Specificity"],3)`|`r round(CM_RFDEF$byClass["Specificity"],3)`|`r round(CM_ANNDEF$byClass["Specificity"],3)`|
|Falsos negativos|`r CM_SVMDEF$table[2,1]`|`r CM_RFDEF$table[2,1]`|`r CM_ANNDEF$table[2,1]`|
|Falsos positivos|`r CM_SVMDEF$table[1,2]`|`r CM_RFDEF$table[1,2]`|`r CM_ANNDEF$table[1,2]`|


  *Comparación de modelos con base de datos reducida*

|Algoritmos|ANN|ANN|SVM||SVM|
|---	|---	|---	|---	|---	|
|Base de datos|Completa|Incompleta|Completa|Incompleta|
|Precisión|`r round(CM_ANNDEF$overall["Accuracy"],3)`|`r round(CM_ANNred$overall["Accuracy"],3)`|`r round(CM_SVMDEF$overall["Accuracy"],3)`|`r round(CM_SVMred$overall["Accuracy"],3)`|
|Sensibilidad|`r round(CM_ANNDEF$byClass["Sensitivity"],3)`|`r round(CM_ANNred$byClass["Sensitivity"],3)`|`r round(CM_SVMDEF$byClass["Sensitivity"],3)`|`r round(CM_SVMred$byClass["Sensitivity"],3)`|
|Especificidad|`r round(CM_ANNDEF$byClass["Specificity"],3)`|`r round(CM_ANNred$byClass["Specificity"],3)`|`r round(CM_SVMDEF$byClass["Specificity"],3)`|`r round(CM_SVMred$byClass["Specificity"],3)`|
|Falsos negativos|`r CM_ANNDEF$table[2,1]`|`r CM_ANNred$table[2,1]`|`r CM_SVMDEF$table[2,1]`|`r CM_SVMred$table[2,1]`|
|Falsos positivos|`r CM_ANNDEF$table[1,2]`|`r CM_ANNred$table[1,2]`|`r CM_SVMDEF$table[1,2]`|`r CM_SVMred$table[1,2]`|
